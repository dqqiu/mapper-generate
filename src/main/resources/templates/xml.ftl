...
<mapper namespace="${modules['mapper'].targetPackage}.${table.getFirstLetterUpperName()}${modules['mapper'].objectNameSuffix}">
	...
	<#list table.fields as field>
		...
		<result property="${field.camelName}Begin" column="${field.name}Begin" />
		<result property="${field.camelName}End" column="${field.name}End" />
		</#if>
	</#list>
	</resultMap>
	...
	<insert id="insertBySelective" parameterType="${modules['model'].targetPackage}.${table.getFirstLetterUpperName()}${modules['model'].objectNameSuffix}">
		...
		<#list table.fields as field>
			<if test="${field.camelName} != null && ${field.camelName} != ''">
				${field.name},
			</if>
		</#list>
		</trim>
	...
	<!-- 根据条件查询记录 -->
	<select id="getByCondition" resultMap="${table.camelName}" parameterType="${modules['vo'].targetPackage}.${table.getFirstLetterUpperName()}${modules['vo'].objectNameSuffix}">
		SELECT
		<include refid="columns" />
		FROM ${table.name}
	...
	<!-- 获取符合条件的记录数 -->
	<select id="getCountByCondition" parameterType="${modules['vo'].targetPackage}.${table.getFirstLetterUpperName()}${modules['vo'].objectNameSuffix}">
		SELECT
		count(*)
		FROM ${table.name}
		<include refid="condition" />
